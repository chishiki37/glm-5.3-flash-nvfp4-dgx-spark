#!/usr/bin/env bash
# Parameterized GLM TP2 launcher for autoresearch sweep.
# Usage: launch_tp2_param.sh <0|1>
# Config via env (must match across ranks):
#   GLM_KV_DTYPE   auto|fp8_e4m3        (default auto)
#   GLM_KV_MEM     ""|<bytes>           (default "")
#   GLM_SPEC       0|3|4                MTP num_speculative_tokens (0 = off)
set -euo pipefail
NODE_RANK="${1:?usage: launch_tp2_param.sh <0|1>}"
GLM_KV_DTYPE="${GLM_KV_DTYPE:-auto}"
GLM_KV_MEM="${GLM_KV_MEM:-}"
GLM_SPEC="${GLM_SPEC:-0}"
...
echo "launched $NAME rank=$NODE_RANK kv=$GLM_KV_DTYPE kvmem=$GLM_KV_MEM spec=$GLM_SPEC"
sleep 2
