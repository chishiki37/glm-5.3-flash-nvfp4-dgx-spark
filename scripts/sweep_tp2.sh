...
export BENCH_URL BENCH_MODEL=glm-5.3-flash
DIR="$(cd "$(dirname "$0")" && pwd)"
RESULTS="$DIR/sweep_tp2_results.jsonl"
MARKER_DIR="$DIR/sweep_markers"
mkdir -p "$MARKER_DIR"
...
launch_config() { # $1 kv_dtype  $2 kv_mem  $3 spec
  ...
  timeout 120 ssh -o ConnectTimeout=10 "$WORK" \
    "GLM_KV_DTYPE=$kv GLM_KV_MEM=$kvm GLM_SPEC=$spec bash ~/launch_tp2_param.sh 1" || return 1
  sleep 25
  echo "[$(date +%H:%M)] launch head (rank0)"
  timeout 120 ssh -o ConnectTimeout=10 "$HEAD" \
    "GLM_KV_DTYPE=$kv GLM_KV_MEM=$kvm GLM_SPEC=$spec bash ~/launch_tp2_param.sh 0" || return 1
...
probe_and_bank() { # $1 name  $2 kv  $3 kvm  $4 spec
  ...
  local probe
  probe=$(timeout 2700 ssh -o ConnectTimeout=10 "$HEAD" 'bash ~/probe_onbox.sh')
  local rc=$?
  local accept=""
  if [ $rc -eq 0 ]; then
